...
// Fix the API_BASE_URL - change from 5000 to 3001 and remove /api
const API_BASE_URL = import.meta.env.VITE_BACKEND_API_URL || "http://localhost:3001" // Changed from 5000 to 3001
...
export async function fetchMovies(query = "", genre = "", year = "", sortBy = "", rating = "") {
  ...
  if (query) {
    if (genre) {
      results = results.filter((movie) => movie.genre_ids.includes(Number.parseInt(genre)))
    }
    if (year) {
    ...
    if (sortBy && typeof sortBy === "string" && sortBy.includes(".")) {
      ...
      results.sort((a, b) => (order === "desc" ? b[field] - a[field] : a[field] - b[field]))
    }

    if (rating) {
      results = results.filter((movie) => movie.vote_average >= Number.parseFloat(rating))
    }
  }
