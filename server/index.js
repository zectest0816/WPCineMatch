...
mongoose
  .connect("mongodb://127.0.0.1:27017/cinematch")
...
app.patch("/comments/:commentId", async (req, res) => {
  ...
  const updated = await CommentModel.findByIdAndUpdate(req.params.commentId, { text, rating }, { new: true })
  res.json(updated)
})
...
// NEW ROUTE: Get user by email (for profile page)
app.get("/users/by-email/:email", async (req, res) => {
  try {
    const { email } = req.params
    console.log("Looking for user with email:", email)

    // Try to find user in CinematchModel first (your login collection)
    let user = await CinematchModel.findOne({ email }).select("-password")
    console.log("Found in CinematchModel:", user)

    // If not found, try UserModel
    if (!user) {
      user = await UserModel.findOne({ email }).select("-password")
      console.log("Found in UserModel:", user)
    }
...
app.patch("/users/:userId", async (req, res) => {
  try {
    ...
    const user = await UserModel.findByIdAndUpdate(req.params.userId, updateData, { new: true, runValidators: true })
